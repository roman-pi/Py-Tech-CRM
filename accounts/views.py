...
class CreateAccountView(LoginRequiredMixin, CreateView):
    ...
    def form_valid(self, form):
        ...
        for i in form.cleaned_data['contacts']:
            con = Contact.objects.get(first_name=i)
            account_object.contacts.add(con)
...
class AccountUpdateView(LoginRequiredMixin, UpdateView):
    ...
    def form_valid(self, form):
        ...
        if self.request.POST.getlist('contacts', []):
            ...
            for account in self.request.POST.getlist('contacts'):
                temp = ''.join(account)
                account_object.contacts.add(Contact.objects.get(first_name=temp)) #Contact.objects.values_list(flat=True).filter(first_name=self.request.POST.get('contacts'))
        if self.request.POST.getlist('assigned_to', []):
