...
from common.forms import (
    ...
    APISettingsForm, FilterForm, NameForm
)
...
from common.utils import ROLES, STAGES, CURRENCY_CODES, STAGE_COLOR
...
class CRMcalculations():
    ...
    #             return HttpResponseRedirect('/common/your_name/')
    #
    ...
    #         # check whether it's valid:
    #         if form.is_valid():
    #             filter = form.cleaned_data['filter_by']
    #             args = {'form': form, 'filter':filter}
    #         else:
    #             args = {'form': form, 'filter': "no "}
    #
    ...
    def by_date_filter(self, set, date_field, days):  #date_field input must be a string: 'example'
        filter_date = datetime.datetime.now()-datetime.timedelta(days=days)
        filter = date_field + '__gte'
        filtered_ids = set.values_list('id', flat=True).filter(**{filter: filter_date})
        empty_ids = set.values_list('id',  flat=True).filter(**{date_field: None})
        both = list(chain(filtered_ids, empty_ids))
        # print(filtered_ids)
        return set.filter(pk__in=both)

    def filtered_closed_opportunities(self, set, date_field, filtered_stage,  days):  #date_field and filtered stage must be a string: 'example'
        closed = CRMcalculations().by_date_filter(set.filter(stage=filtered_stage), date_field, days)
        not_closed = set.exclude(stage=filtered_stage)
        return list(chain(closed, not_closed))

    def max_opportunity_count(self):
        maxop = 0
        for s in STAGES:
            amounts = CRMcalculations().by_date_filter(Opportunity.objects.all(), 'closed_on', 0).filter(stage=s[0])

            # print(Opportunity._meta.get_fields.filter(name='closed_on'))

            if maxop <= len(amounts):
                maxop = len(amounts)
        return maxop

    def empty_space_count(self, i):
        temp = ''
        for j in range(i):
            temp += 'i'
        return temp

    def amounts_by_currency(self, stage, days):
        stage_amounts = []
        for c in CURRENCY_CODES:
            sum = 0
            temp_amounts = CRMcalculations().by_date_filter(Opportunity.objects.all(), 'closed_on',  days).values_list('amount', flat=True).filter(stage=stage[0], currency=c[0])
            if len(temp_amounts) != 0:
                for t in temp_amounts:
                    sum += t
                stage_amounts.append(sum)
                stage_amounts.append(c[0])
                # print(StageAmounts(stage=stage[0], currencies=stage_amounts).currencies)
        return StageAmounts(stage=stage[0], currencies=stage_amounts)


    def stage_info(self, stage, stages, i):
        maxop = CRMcalculations().max_opportunity_count()
        st = []
        st.append(stages[i][0])  # stage.0 (name of stage)
        st.append(CRMcalculations().empty_space_count(i))  # stage.1 (amount of empty columns before stage)
        st.append(CRMcalculations().empty_space_count(len(stages) - 1 - i))  # stage.2 (amount of empty columns after stage)
        amounts = Opportunity.objects.values_list('amount', flat=True).filter(stage=stage[0])
        st.append(CRMcalculations().empty_space_count(maxop - len(amounts)))  # stage.3 (amount of empty columns for horizontal table)
        st.append(CRMcalculations().amounts_by_currency(stage, 7))  # stage.4 (amounts by currency filtered by date)
        st.append(STAGE_COLOR[i][1]) #stage.5 (stage color)
        return st
...
class HomeView(LoginRequiredMixin, TemplateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        i = 0
        for s in STAGES:
            stages.append(CRMcalculations().stage_info(s, STAGES, i))
            i+=1
        #name_of_the_page()
        # print(request.POST['date_drop'])
        # datee=CRMcalculations().by_date_filter(Opportunity.objects.all(), 'closed_on', 1)
        # print(datee)
        ...
        context["filtered_opportunities"] = CRMcalculations().filtered_closed_opportunities(opportunities, 'closed_on', 'CLOSED LOST', 7) #<---AMOUNT OF DAYS FILTERED IS DEFINED HERE
        return context
